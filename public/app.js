...
function publicHome(){
  app.innerHTML=`<section class="hero"><div><p class="eyebrow">Team Vicov · Bucovina</p><h1>Pasiune pentru zbor.<br><em>Respect pentru origini.</em></h1><p>Descoperă porumbeii Team Vicov, rezultatele lor și generațiile din spatele fiecărui pedigree.</p><a class="button" href="#catalog">Descoperă porumbeii <span aria-hidden="true">↗</span></a></div><div class="seal" aria-hidden="true"><strong>TV</strong><span>TEAM VICOV</span><span>BUCOVINA · ROMÂNIA</span></div></section><div class="introline"><span>Porumbei voiajori & pedigree</span><span>Origini documentate. Pasiune în fiecare generație.</span></div><section id="catalog"><div class="section-head"><div><p class="eyebrow">Registrul Team Vicov</p><h2>Porumbeii noștri <span class="count">${birds.length}</span></h2></div><input id="search" class="search" type="search" aria-label="Caută porumbei" placeholder="Caută după serie sau nume…"></div><div class="cards" id="bird-list"></div></section><section class="upcoming" id="licitatii"><div><p class="eyebrow">Licitații Team Vicov</p><h2>Următoarea generație începe aici.</h2><p>Licitațiile vor fi anunțate aici după lansarea modulului de licitare.</p></div><span class="badge draft">În pregătire</span></section>`;
  document.querySelector('#search').addEventListener('input',renderList);renderList();
}
...
function renderList(){
  ...
  const filtered=birds.filter(b=>[b.ring,b.name,adminPage?b.owner:''].join(' ').toLocaleLowerCase('ro').includes(query));
  const container=document.querySelector('#bird-list');
  if(!filtered.length){container.innerHTML=`<div class="empty"><h3>${query?'Niciun rezultat':adminPage?'Primul porumbel, prima poveste.':'Catalogul se pregătește.'}</h3><p>${query?'Încearcă o altă serie sau un alt nume.':adminPage?'Adaugă un porumbel, apoi completează fotografia, rezultatele și legăturile cu părinții.':'Vom publica aici porumbeii, fotografiile și pedigree-urile Team Vicov.'}</p></div>`;return;}
